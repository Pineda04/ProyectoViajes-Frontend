...
export const DestinationsPage = () => {
  const destinationsData = useDestinationsStore((state) => state.destinationsData);
  const loadData = useDestinationsStore((state) => state.loadData);
  const nextPage = useDestinationsStore((state) => state.nextPage);
  const previousPage = useDestinationsStore((state) => state.previousPage);
  ...
  const handleConfirmDelete = async () => {
    if (destinationToDelete) {
      try {
        await useDestinationsStore.getState().deleteDestination(destinationToDelete.id);
      } catch (error) {
      ...
      <div className="sm:flex sm:items-center sm:justify-between mb-4">
        ...
        <div className="flex items-center gap-x-3">
          ...
          >
            <LinkCustom
              label="Nuevo"
              to="/administration/destinations/new"
              icon={
          ...
          <tbody>
            {destinationsData.items.map((destination) => (
              ...
              >
                ...
                <td className="px-6 py-4 truncate" title={destination.description}>
          ...
          >
            ...
            >
              ...
              <p className="text-gray-600 mb-6">
                ¿Está seguro de que desea eliminar el destino{" "}
                {destinationToDelete?.name}? Todos los datos relacionados se
      ...
      <div className="mt-6 sm:flex sm:items-center sm:justify-between">
        <div className="text-sm text-gray-800 dark:text-gray-800">
          ...
          <span className="font-medium text -gray-700 dark:text-gray-800">
            {destinationsData.currentPage} de {destinationsData.totalPages}
          </span>
        </div>
        ...
        <div className="flex items-center mt-4 gap-x-4 sm:mt-0">
          <motion.button
            ...
            disabled={!destinationsData.hasPreviousPage}
            className={`flex items-center justify-center px-5 py-2 text-sm text-gray-700 bg-white border border-gray-300 rounded-md hover:bg-gray-100 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-opacity-50 transition-colors duration-300 
              ${
                !destinationsData.hasPreviousPage
          ...
          <motion.button
            onClick={nextPage}
            disabled={!destinationsData.hasNextPage}
            className={`flex items-center justify-center px-5 py-2 text-sm text-gray-700 bg-white border border-gray-300 rounded-md hover:bg-gray-100 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-opacity-50 transition-colors duration-300
              ${
                !destinationsData.hasNextPage
                  ? "opacity-50 cursor-not-allowed"
                  : ""
              }`}
            variants={buttonVariants}
