...
export const PointsInterestPage = () => {
  const pointsInterestData = usePointsInterestStore((state) => state.pointsInterestData);
  const loadData = usePointsInterestStore((state) => state.loadData);
  const nextPage = usePointsInterestStore((state) => state.nextPage);
  const previousPage = usePointsInterestStore((state) => state.previousPage);
  const [isLoading, setIsLoading] = useState(true);
  const [searchTerm, setSearchTerm] = useState("");

  const [isDeleteModalOpen, setIsDeleteModalOpen] = useState(false);
  const [pointInterestToDelete, setPointInterestToDelete] = useState(null);
  ...
  const openDeleteModal = (pointInterest) => {
    setPointInterestToDelete(pointInterest);
    setIsDeleteModalOpen(true);
  };

  const closeDeleteModal = () => {
    setIsDeleteModalOpen(false);
    setPointInterestToDelete(null);
  };

  const handleConfirmDelete = async () => {
    if (pointInterestToDelete) {
      try {
        await usePointsInterestStore.getState().deletePointInterest(pointInterestToDelete.id);
      } catch (error) {
    ...
    <>
      <div className="sm:flex sm:items-center sm:justify-between mb-4">
        ...
        <div className="flex items-center gap-x-3">
          ...
          >
            <LinkCustom
              label="Nuevo"
              to="/administration/points-interest/new"
              icon={
      ...
      <div className="relative overflow-x-auto shadow-md sm:rounded-lg">
        <table className="w-full text-sm text-left rtl:text-right text-gray-500 dark:text-gray-400 table-fixed">
          ...
          <tbody>
            {pointsInterestData.items.map((pointInterest) => (
              <tr
                key={pointInterest.id}
              ...
              >
                ...
                <td className="px-6 py-4 truncate" title={pointInterest.description}>
                  {pointInterest.description}
                </td>
                <td className="px-6 py-4 text-right">
                  <div className="flex justify-end gap-2">
                    ...
                    >
                      <button
                        onClick={() => navigate(`edit/${pointInterest.id}`)}
                    ...
                    >
                      <button
                        onClick={() => openDeleteModal(pointInterest)}
          ...
          >
            ...
            >
              ...
              <p className="text-gray-600 mb-6">
                ¿Está seguro de que desea eliminar el punto de interés{" "}
                {pointInterestToDelete?.name}? Todos los datos relacionados se
      ...
      <div className="mt-6 sm:flex sm:items-center sm:justify-between">
        <div className="text-sm text-gray-800 dark:text-gray-800">
          ...
          <span className="font-medium text -gray-700 dark:text-gray-800">
            {pointsInterestData.currentPage} de {pointsInterestData.totalPages}
          </span>
        </div>
        ...
        <div className="flex items-center mt-4 gap-x-4 sm:mt-0">
          <motion.button
            ...
            disabled={!pointsInterestData.hasPreviousPage}
            className={`flex items-center justify-center px-5 py-2 text-sm text-gray-700 bg-white border border-gray-300 rounded-md hover:bg-gray-100 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-opacity-50 transition-colors duration-300 
              ${
                !pointsInterestData.hasPreviousPage
          ...
          <motion.button
            onClick={nextPage}
            disabled={!pointsInterestData.hasNextPage}
            className={`flex items-center justify-center px-5 py-2 text-sm text-gray-700 bg-white border border-gray-300 rounded-md hover:bg-gray-100 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-opacity-50 transition-colors duration-300
              ${
                !pointsInterestData.hasNextPage
                  ? "opacity-50 cursor-not-allowed"
                  : ""
              }`}
            variants={buttonVariants}
