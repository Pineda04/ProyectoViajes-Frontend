...
export const FlightsPage = () => {
  const flightsData = useFlightsStore((state) => state.flightsData);
  const loadData = useFlightsStore((state) => state.loadData);
  const nextPage = useFlightsStore((state) => state.nextPage);
  const previousPage = useFlightsStore((state) => state.previousPage);
  ...
  const handleConfirmDelete = async () => {
    if (flightToDelete) {
      try {
        await useFlightsStore.getState().deleteFlight(flightToDelete.id);
      } catch (error) {
          ...
          >
            ...
            >
              ...
              <p className="text-gray-600 mb-6">
                ¿Está seguro de que desea eliminar el vuelo{" "}
                {flightToDelete?.name}? Todos los datos relacionados se
      ...
      <div className="mt-6 sm:flex sm:items-center sm:justify-between">
        <div className="text-sm text-gray-800 dark:text-gray-800">
          ...
          <span className="font-medium text -gray-700 dark:text-gray-800">
            {flightsData.currentPage} de {flightsData.totalPages}
          </span>
        </div>
        ...
        <div className="flex items-center mt-4 gap-x-4 sm:mt-0">
          <motion.button
            ...
            disabled={!flightsData.hasPreviousPage}
          ...
          <motion.button
            onClick={nextPage}
            disabled={!flightsData.hasNextPage}
            className={`flex items-center justify-center px-5 py-2 text-sm text-gray-700 bg-white border border-gray-300 rounded-md hover:bg-gray-100 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-opacity-50 transition-colors duration-300
              ${
                !flightsData.hasNextPage
                  ? "opacity-50 cursor-not-allowed"
                  : ""
              }`}
            variants={buttonVariants}
