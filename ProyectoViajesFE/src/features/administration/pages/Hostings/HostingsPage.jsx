...
import { useHostingsStore } from "../../store/useHostingsStore";
...
export const HostingsPage = () => {
  const hostingsData = useHostingsStore((state) => state.hostingsData);
  const loadData = useHostingsStore((state) => state.loadData);
  const nextPage = useHostingsStore((state) => state.nextPage);
  const previousPage = useHostingsStore((state) => state.previousPage);
  ...
  const handleConfirmDelete = async () => {
    if (hostingToDelete) {
      try {
        await useHostingsStore.getState().deleteHosting(hostingToDelete.id);
      } catch (error) {
    ...
    <>
      <div className="sm:flex sm:items-center sm:justify-between mb-4">
        ...
        <div className="flex items-center gap-x-3">
          ...
          >
            <LinkCustom
              label="Nuevo"
              to="/administration/hostings/new"
              icon={
      ...
      <div className="relative overflow-x-auto shadow-md sm:rounded-lg">
        <table className="w-full text-sm text-left rtl:text-right text-gray-500 dark:text-gray-400 table-fixed">
          ...
          <tbody>
            {hostingsData.items.map((hosting) => (
              <tr
                key={hosting.id}
              ...
              >
                ...
                <td className="px-6 py-4 truncate" title={hosting.description}>
                  {hosting.description}
                </td>
                <td className="px-6 py-4 text-right">
                  <div className="flex justify-end gap-2">
                    ...
                    >
                      <button
                        onClick={() => navigate(`edit/${hosting.id}`)}
          ...
          >
            ...
            >
              ...
              <p className="text-gray-600 mb-6">
                ¿Está seguro de que desea eliminar el hospedaje{" "}
                {hostingToDelete?.name}? Todos los datos relacionados se
      ...
      <div className="mt-6 sm:flex sm:items-center sm:justify-between">
        <div className="text-sm text-gray-800 dark:text-gray-800">
          ...
          <span className="font-medium text -gray-700 dark:text-gray-800">
            {hostingsData.currentPage} de {hostingsData.totalPages}
          </span>
        </div>
        ...
        <div className="flex items-center mt-4 gap-x-4 sm:mt-0">
          <motion.button
            ...
            disabled={!hostingsData.hasPreviousPage}
          ...
          <motion.button
            onClick={nextPage}
            disabled={!hostingsData.hasNextPage}
            className={`flex items-center justify-center px-5 py-2 text-sm text-gray-700 bg-white border border-gray-300 rounded-md hover:bg-gray-100 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-opacity-50 transition-colors duration-300
              ${
                !hostingsData.hasNextPage
                  ? "opacity-50 cursor-not-allowed"
                  : ""
              }`}
            variants={buttonVariants}
