...
export const EditHostingPage = () => {
  const navigate = useNavigate();
  const { id } = useParams();
  const selectedHosting = useHostingsStore((state) => state.selectedHosting);
  const getHosting = useHostingsStore((state) => state.getHosting);
  const updateHosting = useHostingsStore((state) => state.updateHosting);
  ...
  const formik = useFormik({
    ...
    onSubmit: async (formValues) => {
      try {
        setIsLoading(true);
        const result = await updateHosting(formValues);
        if (result.status) {
          navigate('/administration/hostings');
        } else {
          setError(result.message || "Error al actualizar el hospedaje");
        }
      } catch (error) {
        setError("Error al actualizar el hospedaje");
      } finally {
        setIsLoading(false);
      }
    }
  ...
  useEffect(() => {
    const loadHosting = async () => {
      try {
        setIsLoading(true);
        const result = await getHosting(id);
        ...
        console.error(error);
        setError("Error al cargar el hospedaje");
      } finally {
        setIsLoading(false);
      }
    };
