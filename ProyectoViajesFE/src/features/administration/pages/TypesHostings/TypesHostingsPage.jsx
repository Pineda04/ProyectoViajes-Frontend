...
import { useTypesHostingsStore } from "../../store/useTypesHostingsStore";
...
export const TypesHostingsPage = () => {
  const typesHostingsData = useTypesHostingsStore((state) => state.typesHostingsData);
  const loadData = useTypesHostingsStore((state) => state.loadData);
  const nextPage = useTypesHostingsStore((state) => state.nextPage);
  const previousPage = useTypesHostingsStore((state) => state.previousPage);
  const [isLoading, setIsLoading] = useState(true);
  const [searchTerm, setSearchTerm] = useState("");

  const [isDeleteModalOpen, setIsDeleteModalOpen] = useState(false);
  const [typeHostingToDelete, setTypeHostingToDelete] = useState(null);
  ...
  const openDeleteModal = (typeHosting) => {
    setTypeHostingToDelete(typeHosting);
    setIsDeleteModalOpen(true);
  };

  const closeDeleteModal = () => {
    setIsDeleteModalOpen(false);
    setTypeHostingToDelete(null);
  };

  const handleConfirmDelete = async () => {
    if (typeHostingToDelete) {
      try {
        await useTypesHostingsStore.getState().deleteTypeHosting(typeHostingToDelete.id);
      } catch (error) {
        console.error("Error al eliminar el tipo de hospedaje:", error);
      }
    ...
    <>
      <div className="sm:flex sm:items-center sm:justify-between mb-4">
        <div>
          <form onSubmit={handleSearch} className="flex gap-2 items-center">
            <input
              ...
              placeholder="Buscar tipos de hospedaje..."
        ...
        <div className="flex items-center gap-x-3">
          ...
          >
            <LinkCustom
              label="Nuevo"
              to="/administration/types-hostings/new"
              icon={
      ...
      <div className="relative overflow-x-auto shadow-md sm:rounded-lg">
        <table className="w-full text-sm text-left rtl:text-right text-gray-500 dark:text-gray-400 table-fixed">
          ...
          <tbody>
            {typesHostingsData.items.map((typeHosting) => (
              <tr
                key={typeHosting.id}
              ...
              >
                ...
                <td className="px-6 py-4 text-right">
                  <div className="flex justify-end gap-2">
                    ...
                    >
                      <button
                        onClick={() => navigate(`edit/${typeHosting.id}`)}
                    ...
                    >
                      <button
                        onClick={() => openDeleteModal(typeHosting)}
          ...
          >
            ...
            >
              <h2 className="text-2xl font-bold text-gray-900 mb-4">
                Eliminar Tipo de hospedaje
              </h2>
              <p className="text-gray-600 mb-6">
                ¿Está seguro de que desea eliminar el tipo de hospedaje{" "}
                {typeHostingToDelete?.name}? Todos los datos relacionados se
      ...
      <div className="mt-6 sm:flex sm:items-center sm:justify-between">
        <div className="text-sm text-gray-800 dark:text-gray-800">
          ...
          <span className="font-medium text -gray-700 dark:text-gray-800">
            {typesHostingsData.currentPage} de {typesHostingsData.totalPages}
          </span>
        </div>
        ...
        <div className="flex items-center mt-4 gap-x-4 sm:mt-0">
          <motion.button
            ...
            disabled={!typesHostingsData.hasPreviousPage}
            className={`flex items-center justify-center px-5 py-2 text-sm text-gray-700 bg-white border border-gray-300 rounded-md hover:bg-gray-100 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-opacity-50 transition-colors duration-300 
              ${
                !typesHostingsData.hasPreviousPage
          ...
          <motion.button
            onClick={nextPage}
            disabled={!typesHostingsData.hasNextPage}
            className={`flex items-center justify-center px-5 py-2 text-sm text-gray-700 bg-white border border-gray-300 rounded-md hover:bg-gray-100 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-opacity-50 transition-colors duration-300
              ${
                !typesHostingsData.hasNextPage
                  ? "opacity-50 cursor-not-allowed"
                  : ""
              }`}
            variants={buttonVariants}
