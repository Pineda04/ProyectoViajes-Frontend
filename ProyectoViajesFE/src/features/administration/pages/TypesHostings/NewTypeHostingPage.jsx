...
export const NewTypeHostingPage = () => {
  ...
  const formik = useFormik({
    ...
    onSubmit: async (formValues) => {
      try {
        setIsLoading(true);
        const { data } = await viajesApi.post('/types_hosting', formValues);
        
        if (data.status) {
          navigate('/administration/types-hostings');
        } else {
          setError(data.message || "Error al crear el tipo de hospedaje");
        }
      } catch (error) {
        console.error(error);
        setError("Error al crear el tipo de hospedaje");
      } finally {
        setIsLoading(false);
      }
    }
