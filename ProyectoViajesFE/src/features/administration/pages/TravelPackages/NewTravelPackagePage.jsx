...
export const NewTravelPackagePage = () => {
  ...
  const formik = useFormik({
    ...
    onSubmit: async (formValues) => {
      try {
        setIsLoading(true);
        const { data } = await viajesApi.post('/travel_packages', formValues);
        
        if (data.status) {
          navigate('/administration/travel-packages');
        } else {
          setError(data.message || "Error al crear el paquete de viaje");
        }
      } catch (error) {
        console.error(error);
        setError("Error al crear el paquete de viaje");
      } finally {
        setIsLoading(false);
      }
    }
