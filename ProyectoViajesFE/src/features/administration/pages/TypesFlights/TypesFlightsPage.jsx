...
import { useTypesFlightsStore } from "../../store/useTypesFlightsStore";
...
export const TypesFlightsPage = () => {
  const typesFlightsData = useTypesFlightsStore((state) => state.typesFlightsData);
  const loadData = useTypesFlightsStore((state) => state.loadData);
  const nextPage = useTypesFlightsStore((state) => state.nextPage);
  const previousPage = useTypesFlightsStore((state) => state.previousPage);
  const [isLoading, setIsLoading] = useState(true);
  const [searchTerm, setSearchTerm] = useState("");

  const [isDeleteModalOpen, setIsDeleteModalOpen] = useState(false);
  const [typeFlightToDelete, setTypeFlightToDelete] = useState(null);
  ...
  const openDeleteModal = (typeFlight) => {
    setTypeFlightToDelete(typeFlight);
    setIsDeleteModalOpen(true);
  };

  const closeDeleteModal = () => {
    setIsDeleteModalOpen(false);
    setTypeFlightToDelete(null);
  };

  const handleConfirmDelete = async () => {
    if (typeFlightToDelete) {
      try {
        await useTypesFlightsStore.getState().deleteTypeFlight(typeFlightToDelete.id);
      } catch (error) {
        console.error("Error al eliminar el tipo de vuelo:", error);
      }
    ...
    <>
      <div className="sm:flex sm:items-center sm:justify-between mb-4">
        <div>
          <form onSubmit={handleSearch} className="flex gap-2 items-center">
            <input
              ...
              placeholder="Buscar tipos de vuelo..."
        ...
        <div className="flex items-center gap-x-3">
          ...
          >
            <LinkCustom
              label="Nuevo"
              to="/administration/types-flights/new"
              icon={
      ...
      <div className="relative overflow-x-auto shadow-md sm:rounded-lg">
        <table className="w-full text-sm text-left rtl:text-right text-gray-500 dark:text-gray-400 table-fixed">
          ...
          <tbody>
            {typesFlightsData.items.map((typeFlight) => (
              <tr
                key={typeFlight.id}
              ...
              >
                ...
                <td className="px-6 py-4 text-right">
                  <div className="flex justify-end gap-2">
                    ...
                    >
                      <button
                        onClick={() => navigate(`edit/${typeFlight.id}`)}
                    ...
                    >
                      <button
                        onClick={() => openDeleteModal(typeFlight)}
          ...
          >
            ...
            >
              <h2 className="text-2xl font-bold text-gray-900 mb-4">
                Eliminar Tipo de vuelo
              </h2>
              <p className="text-gray-600 mb-6">
                ¿Está seguro de que desea eliminar el tipo de vuelo{" "}
                {typeFlightToDelete?.name}? Todos los datos relacionados se
      ...
      <div className="mt-6 sm:flex sm:items-center sm:justify-between">
        <div className="text-sm text-gray-800 dark:text-gray-800">
          ...
          <span className="font-medium text -gray-700 dark:text-gray-800">
            {typesFlightsData.currentPage} de {typesFlightsData.totalPages}
          </span>
        </div>
        ...
        <div className="flex items-center mt-4 gap-x-4 sm:mt-0">
          <motion.button
            ...
            disabled={!typesFlightsData.hasPreviousPage}
            className={`flex items-center justify-center px-5 py-2 text-sm text-gray-700 bg-white border border-gray-300 rounded-md hover:bg-gray-100 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-opacity-50 transition-colors duration-300 
              ${
                !typesFlightsData.hasPreviousPage
          ...
          <motion.button
            onClick={nextPage}
            disabled={!typesFlightsData.hasNextPage}
            className={`flex items-center justify-center px-5 py-2 text-sm text-gray-700 bg-white border border-gray-300 rounded-md hover:bg-gray-100 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-opacity-50 transition-colors duration-300
              ${
                !typesFlightsData.hasNextPage
                  ? "opacity-50 cursor-not-allowed"
                  : ""
              }`}
            variants={buttonVariants}
