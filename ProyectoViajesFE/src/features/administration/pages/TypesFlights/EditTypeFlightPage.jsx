...
export const EditTypeFlightPage = () => {
  const navigate = useNavigate();
  const { id } = useParams();
  const selectedTypeFlight = useTypesFlightsStore((state) => state.selectedTypeFlight);
  const getTypeFlight = useTypesFlightsStore((state) => state.getTypeFlight);
  const updateTypeFlight = useTypesFlightsStore((state) => state.updateTypeFlight);
  ...
  const formik = useFormik({
    ...
    onSubmit: async (formValues) => {
      try {
        setIsLoading(true);
        const result = await updateTypeFlight(formValues);
        if (result.status) {
          navigate('/administration/types-flights');
        } else {
          setError(result.message || "Error al actualizar el tipo de vuelo");
        }
      } catch (error) {
        setError("Error al actualizar el tipo de vuelo");
      } finally {
        setIsLoading(false);
      }
    }
  ...
  useEffect(() => {
    const loadTypeFlight = async () => {
      try {
        setIsLoading(true);
        const result = await getTypeFlight(id);
        ...
        if (result?.status && result.data) {
          ...
          setError("No se pudo cargar el tipo de vuelo");
        }
      } catch (error) {
        console.error(error);
        setError("Error al cargar el tipo de vuelo");
      } finally {
        setIsLoading(false);
      }
    };
